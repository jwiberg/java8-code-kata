...
public class Exercise5Test extends ClassicOnlineStore {
    ...
    public void oldestCustomer() {
         ...
         */
        Optional<Customer> oldestCustomer = customerList.stream().collect(
                Collectors.maxBy(Comparator.comparingInt(customer -> customer.getAge())));
    ...
    public void ageDistribution() {
         ...
         */
        Map<Integer, Long> ageDistribution = customerList.stream().collect(
                Collectors.groupingBy(Customer::getAge, Collectors.counting()));
